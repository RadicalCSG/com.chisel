...
namespace Chisel.Core
{
    ...
    { 
        ...
        void CopyFrom(NativeList<UnsafeList<Edge>> dst, int index, ref BrushIntersectionLoop brushIntersectionLoop, HashedVertices hashedTreeSpaceVertices, int extraCapacity)
        {
			...
			var srcIndices = new NativeArray<ushort>(loopVertexCount, Allocator.Temp);
            try
            {
        ...
        public void Execute(int index)
        {
			...
			var basePolygonSurfaceInfos = new NativeArray<IndexSurfaceInfo>(surfaceCount, Allocator.Temp);
			//NativeCollectionHelpers.EnsureMinimumSize(ref basePolygonSurfaceInfos, surfaceCount);
			var basePolygonEdges = new NativeList<UnsafeList<Edge>>(surfaceCount, Allocator.Temp);
			...
			try
            {
                ...
                {
					var brushIntersections = new NativeList<BrushIntersectionLoop>(intersectionCount, Allocator.Temp);
					//NativeCollectionHelpers.EnsureCapacityAndClear(ref brushIntersections, intersectionCount);
                    var usedNodeOrders = new NativeBitArray(maxNodeOrder, Allocator.Temp);
                    ...
                    try
                    {
						...
						var intersectionEdges = new NativeList<UnsafeList<Edge>>(brushIntersections.Length, Allocator.Temp);
						...
						var intersectionSurfaceSegments = new NativeArray<int2>(surfaceCount + 1, Allocator.Temp);
                        //NativeCollectionHelpers.EnsureMinimumSize(ref intersectionSurfaceSegments, surfaceCount + 1);
                        var intersectionSurfaceInfos = new NativeList<IndexSurfaceInfo>(brushIntersections.Length, Allocator.Temp);
                                               ...
                                               ref UnsafeList<Edge> edges)
        {
            ...
            var newSelfEdges = new NativeArray<Edge>(inputEdgesLength, Allocator.Temp);
                                                      ...
                                                      [NoAlias, ReadOnly] HashedVertices      combinedVertices)
        {
			...
			var newSelfEdges = new NativeArray<Edge>(newSelfEdgesLength, Allocator.Temp);
                                           ...
                                           [NoAlias, ReadOnly] HashedVertices combinedVertices)
        {
			...
			var otherVertices = new NativeArray<ushort>(otherEdges.Length, Allocator.Temp);
            ...
            { 
				...
				var tempList         = new NativeList<ushort> (tempListCapacity, Allocator.Temp);
